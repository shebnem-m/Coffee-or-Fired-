...
const gameoverTitleEl = document.getElementById('gameover-title') || document.querySelector('#gameover-screen h1');
 
// ── Game State ────────────────────────────
let state = 'start'; 
let score, highScore, coffeeLevel, animFrame, currentLevel = 1;
...
// 🎨 General Player Sprite Drawing Function
...
// ══════════════════════════════════════════
//    GAME INIT & LEVEL SYSTEM
// ══════════════════════════════════════════
function setLevelDifficulty(lvl) {
  currentLevel = lvl;
  COFFEE_DRAIN_RATE = 0.007 + (lvl * 0.002); 
...
function checkCollisions() {
  ...
  if (Math.abs(ppx - cx) < 26 && Math.abs(ppy - cy) < 24) {
    ...
    if (score > 0 && Math.floor(score / 100) > currentLevel - 1) { 
      currentLevel++;
      if (currentLevel >= 15) { triggerWinGame(); return; }
      setLevelDifficulty(currentLevel);
...
// ══════════════════════════════════════════
//  🔥 DYNAMIC GAME OVER STATS INJECTOR
// ══════════════════════════════════════════
function injectEndLevelStats(isWin = false) {
  let career = getCareerDetails(currentLevel);
  
  // Find or create the dynamic stats container right above the restart button
  ...
  // Brief timeout to let the DOM settle before drawing the character sprite
  setTimeout(() => {
...
function endGame() {
  ...
  if (gameoverTitleEl) {
    if (score > highScore) { gameoverTitleEl.textContent = "🏆 NEW RECORD! 🏆"; gameoverTitleEl.style.color = "#f1c40f"; } 
    else { gameoverTitleEl.textContent = "💼 GAME OVER 💼"; gameoverTitleEl.style.color = "#e74c3c"; }
  }
  ...
  injectEndLevelStats(false); // Injects data automatically upon defeat
  showScreen('gameover');
}
...
function triggerWinGame() {
  ...
  if (gameoverTitleEl) {
    gameoverTitleEl.textContent = "🏆 YOU ABSOLUTELY WIN! 🏆";
    gameoverTitleEl.style.color = "#f1c40f"; 
  }
  ...
  injectEndLevelStats(true); // Injects data automatically upon victory
  showScreen('gameover');
}
...
startBtn.addEventListener('click', () => { currentLevel = 1; startGame(false); });

continueBtn.addEventListener('click', () => {
  if (continueBtn.style.display !== 'none' && !continueBtn.classList.contains('locked')) {
...
restartBtn.addEventListener('click', () => { if (animFrame) cancelAnimationFrame(animFrame); showScreen('start'); });
 
window.addEventListener('load', () => {
  startHSEl.textContent = localStorage.getItem('officeVsCoffeeHS') || '0';
  const savedScore = localStorage.getItem('officeVsCoffee_savedScore');
